...
class Movie(models.Model):
    ...
    long_time = models.IntegerField(
        verbose_name="Длительность", help_text="В минутах"
    )  # Длительность
...
class Employee(models.Model):
    ...
    position = models.ForeignKey(
        Job, on_delete=models.CASCADE, verbose_name="Должность"
    )  # Должность
...
class Session(models.Model):
    ...
    room = models.ForeignKey(
        Room, on_delete=models.CASCADE, verbose_name="Зал", related_name="room_session"
    )  # Зал
...
class TicketPrice(models.Model):
    ...
    session = models.ForeignKey(
        Session, on_delete=models.CASCADE, verbose_name="Сеанс"
    )  # Сеанс
    sector = models.ForeignKey(
        Sector, on_delete=models.CASCADE, verbose_name="Сектор"
    )  # Сектор
...
class Ticket(models.Model):
    ...
    price = models.ForeignKey(
        TicketPrice, on_delete=models.CASCADE, verbose_name="Цена"
    )  # Цена
    seat = models.ForeignKey(
        Seat, on_delete=models.CASCADE, verbose_name="Место"
    )  # Место
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата создания"
    )  # Дата создания
    status = models.CharField(
        max_length=15, choices=STATUS_CHOICES, default="empty", verbose_name="Статус"
    )  # Статус
